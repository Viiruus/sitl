...
const randomizedMoniteurs = usePrioritizedRandomMoniteurs(
  moniteurs,
  'la-brigade-prioritized-moniteur-ids',
)
...
const moniteursItemListStructuredData = computed(() => ({
  ...
  itemListElement: randomizedMoniteurs.value.map((moniteur: any, index: number) => ({
    '@type': 'ListItem',
    position: index + 1,
