...
const { data: aventureData, pending, error: fetchError } = await useFetch(() => (slug.value ? `/api/guides/aventures/${slug.value}` : null), {
  watch: [slug],
})
    ...
    <div class="mx-auto flex min-h-screen max-w-7xl flex-col gap-8 px-6 py-12 lg:flex-row lg:gap-12">
      ...
      <main class="flex-1">
        <div class="rounded-3xl bg-white/5 p-8 ring-1 ring-white/10">
          ...
          <div v-else-if="fetchError" class="mt-6 rounded-2xl border border-red-500/40 bg-red-500/10 p-6 text-sm text-red-100">
            Impossible de charger l’aventure. {{ fetchError?.data?.message || fetchError?.statusMessage || '' }}
          </div>

          <div v-else class="mt-6">
            <MoniteursAventureEditor
              mode="edit"
              :initial-data="aventure"
            />
          </div>
        </div>
      </main>
